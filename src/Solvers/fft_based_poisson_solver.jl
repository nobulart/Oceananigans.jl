...
function FFTBasedPoissonSolver(grid, planner_flag=FFTW.PATIENT)
    ...
    λx = poisson_eigenvalues(eltype(grid), grid.Nx, grid.Lx, 1, TX())
    λy = poisson_eigenvalues(eltype(grid), grid.Ny, grid.Ly, 2, TY())
    λz = poisson_eigenvalues(eltype(grid), grid.Nz, grid.Lz, 3, TZ())
